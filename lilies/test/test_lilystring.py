...
class TestLilyString(unittest.TestCase):
    ...
    def test_integer_casting(self):
        for i in self.integers:
            ...
            as_string = str(i)
            if len(as_string) > 1:
                multicolor = grow(as_string[0], "cyan") + grow(
                    as_string[1:], "white"
                )
    ...
    def test_ljust(self):
        for s in self.strings:
            p = grow(s, "red")
            self.assertEqual(s.ljust(2), wilt(p.ljust(2)))

            self.assertEqual(s.ljust(40), wilt(p.ljust(40)))

            self.assertEqual(s.ljust(40, "a"), wilt(p.ljust(40, "a")))

            self.assertEqual(
                s.ljust(40, "a"), wilt(p.ljust(40, grow("a", "blue")))
            )
    ...
    def test_rjust(self):
        for s in self.strings:
            p = grow(s, "red")
            self.assertEqual(s.rjust(2), wilt(p.rjust(2)))

            self.assertEqual(s.rjust(40), wilt(p.rjust(40)))

            self.assertEqual(s.rjust(40, "a"), wilt(p.rjust(40, "a")))

            self.assertEqual(
                s.rjust(40, "a"), wilt(p.rjust(40, grow("a", "blue")))
            )
    ...
    def test_center(self):
        for s in self.strings:
            p = grow(s, "red")
            self.assertEqual(s.center(2), wilt(p.center(2)))

            self.assertEqual(s.center(40), wilt(p.center(40)))

            self.assertEqual(s.center(40, "a"), wilt(p.center(40, "a")))

            self.assertEqual(
                s.center(40, "a"), wilt(p.center(40, grow("a", "blue")))
            )
    ...
    def test_resize(self):
        ...
        self.assertEqual(one_at_10chars_no_elipsis, wilt(p_one.resize(10)))
        self.assertEqual(
            one_at_10chars_w_elipsis, wilt(p_one.resize(10, add_elipsis=True))
        )
        self.assertEqual(
            one_at_40chars_left, wilt(p_one.resize(40, justify="left"))
        )
        self.assertEqual(
            one_at_40chars_right, wilt(p_one.resize(40, justify="right"))
        )
        self.assertEqual(
            one_at_40chars_center, wilt(p_one.resize(40, justify="center"))
        )
